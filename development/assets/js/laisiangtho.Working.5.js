...
f.exe->Load{o:[fO,config],f:[Database(fD),other]}->Watch{f:[exe,is,get]}->Meta{f:[is,get]}
Database{config,fO}
*/
/*
Common Prototype
...
(function($, uA) {
    ...
    $.fn[f] = function(options) {
        ...
        Core.prototype.Metalink = function(y) {
            ...
            //     window[x[i]] = f(x[i]).is('link').get('href');
            //     console.log('for',i,x[i],y);
            // }
        };
        Core.prototype.Metacontent = function(x) {
            //x=this.arg[0];
            for (var i = 0, len = x.length; i < len; i++) {
                window[x[i]] = f(x[i]).is('meta').get('content');
            }
        };
