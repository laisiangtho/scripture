(function($,win,doc,uA){
var z='laisiangtho', version='1.0.43.2015.7.12';
...
return z={
	agent:function(){
		...
		var device={
			...
			Orientation:{
				handle:function(){
					if((win.innerHeight / win.innerWidth) < 1){
						$(docElement).attr({class:sO.Orientation.landscape});
					} else {
						$(docElement).attr({class:sO.Orientation.portrait});
					}
							...
							if(obj.queryChapter()){
								...
								if(lai.previous.bible==q.bible && lai.previous.booklist==current_booklist && lai.previous.chapter==q.chapter){
									...
									obj.bible(obj.booklist).progress(function(){
										z.MsgLookup.text(z.utility.Num(obj.result.verse));
									}).done(function(){
										...
										o.msg(msg);
										z.MsgLookup.attr('title',q.q).text(z.utility.Num(q.result)).promise().done(function(){
											if(!obj.result.verse){
