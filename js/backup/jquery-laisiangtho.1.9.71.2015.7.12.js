(function($,win,doc,uA){
var z='laisiangtho', version='1.9.71.2015.7.12';
...
return z={
			...
			next:function(){
				if(l7.length){
					...
					z.loading=function(){
						...
						if(!sO.working){
							this.fn(config.loader).html($('body')).on(sO.Click,function(){
								MsgInfo.text(lai[lai.store.query.bible].lang.l.PleaseWait);
							}).promise().done(function(){
								sO.working=this;
							});
						}
					...
					var prepare=function(lC,lN){
						...
						return {
							...
							next:function(){
								...
								this.offline(function(o){
									lai.store.lang[bID].offline=o.status;
									lai[bID].lang=lai.store.lang[bID];
									callback();
								});
							},
							offline:function(callback){
								var o=z.utility.Url(config.id,[bID],config.file.bible);
								if($.isEmptyObject(lai[bID].bible)){
									if(sO.isCordova){
										window.resolveLocalFileSystemURL(o.local, function(){
										...
										db.get({table:bID}).then(function(storeBible){
											if(storeBible){
												...
												callback({status:$.isEmptyObject(lai[bID].bible)?false:true});
											}else{
												callback({status:false});
											}
										});
	...
	bible:function(){
		...
		config.bible.available.forEach(function(bID){
			...
			offlineClass='icon-zip offline',onlineClass='icon-cloud offline', isAvailable=(isOffline)?offlineClass:onlineClass;
