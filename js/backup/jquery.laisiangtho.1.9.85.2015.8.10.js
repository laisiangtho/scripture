(function($,uA){
var z='laisiangtho', version='1.9.85.2015.8.10';
...
return z={
    ...
    database:{
        ...
        UpdateLookUp:function(){
            return this.put({table:config.store.lookup,data:lai.store.lookup});
        }
    },
    load:function(){
    ...
    init:function(){
        var css={result:this.is('result').class}, setting={page:config.page[0],bible:config.bible.available[0],book:1,testament:1,catalog:1,chapter:1,verses:'',verse:'',q:'',result:''};
    ...
    */
    ...
    fA:{
        ...
        chapter:{
            get:{
                next:function(){
                    var bID=parseInt(lai.store.query.book), cID=parseInt(lai.store.query.chapter)+1;
                ...
                previous:function(){
                    var bID=parseInt(lai.store.query.book), cID=parseInt(lai.store.query.chapter)-1;
                    if(cID<1){
                        bID--; bID=(bID<1)?66:bID; cID=bible.info[bID].c;
                    }
                    return {book:bID,chapter:cID};
                }
            },
            ...
            list:function(e){
                ...
                if(container.is(':hidden')){
                    ...
                    z.menu.chapter(lai.store.query.book).appendTo(container.fadeIn(200).find(z.is(z.get(x).class()[1]).id).empty()).promise().done(function(){
            ...
            book:function(bID){
                ...
                return $(h.span).html(z.utility.Num(bible.info[bID].c)).on(sO.Click,function(){
                    ...
                    if(container.children('ol').length){
                        ...
                        z.menu.chapter(bID).appendTo(container).promise().done($(this).addClass(config.css.active));
                    }
                });
            }
        },
        lookup:{
            setting:function(e){					
                ...
                if(container.is(':hidden')){
                    x.addClass(config.css.active);
                    z.menu.lookup(container.fadeIn(200).find(z.is(z.get(x).class()[1]).id).empty()).promise().done(function(){
                        $(document).on(sO.Click,function(y){
                            if(z.container.Closest(container,y,x))z.container.FadeOut(container,x);
                        });
                    });
        ...
        Mobile:{
            ...
            Chapter:function(e){
                ...
                this.Common(e,function(){
                    return {
                        msg:z.menu.chapter(lai.store.query.book)
                    };
                });
            },
            Bookmark:function(e){
    ...
    menu:{
        bible:function(attr,callback){
            var container=$(h.ol,attr);
        ...
        lookup:function(container){
                    ...
                    if(x.get(0).tagName.toLowerCase()==='p'){
                        ...
                        x.toggleClass(config.css.active).promise().done(function(){
                            if(this.hasClass(config.css.active)){
                                lai.store.lookup.setting[sID]=true;
                            }else{
                                delete lai.store.lookup.setting[sID];
                            }
                        });
                    }
                    db.UpdateLookUp();
            ...
            }, container=$(h.ol,{class:'list-lookup'}).appendTo(container), lD=lai.store.lang[lai.store.query.bible];
            $.each(bible.catalog,function(tID,cL){
                var tTD=Object.keys(config.language)[0]+tID, tClass=(lai.store.lookup.setting[tTD]?config.css.active:'testament');
                $(h.li,{id:tTD,class:tClass}).html(
                    $(h.p,{text:lD.t[tID]}).on(sO.Click,function(e){
                    ...
                    $(h.ol,{class:'section'}).appendTo(this).promise().done(function(){
                        ...
                        $.each(cL,function(sID,bL){
                            var sTD=Object.keys(config.language)[1]+sID, sClass=(lai.store.lookup.setting[sTD]?config.css.active:'');
                            $(h.li,{id:sTD,class:sClass}).append(
                                $(h.p,{text:lD.s[sID]}).on(sO.Click,function(e){
                                ...
                                bL.forEach(function(bID){
                                    $(h.li,{id:bID,class:(lai.store.lookup.book[bID]?config.css.active:'')}).text(lD.b[bID]).on(sO.Click,function(){
    ...
    bible:function(){
        ...
        this.menu.bible(attr,callback)
        this.containerMain.done.sortable
        */
        var p=this.utility.Page(1);
        this.containerMain.html(this.menu.bible({id:'dragable',class:'rows row-bible'},function(bID,lang,isLocal,availableClass,offlineClass,onlineClass,isAvailable){
            return $(h.p).append(
            ...
            var ol=this.children();
            Sortable.create(document.getElementById(ol.get(0).id), {
