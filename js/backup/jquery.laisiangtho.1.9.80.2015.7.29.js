(function($,uA){
var z='laisiangtho', version='1.9.80.2015.7.29';
	...
	agent:function(){
		var device={
			...
			get:function(){
				var d=[sO.Extend];
				if(this.mobile()){
					d.push('mobile',sO.Platform);//console.log('//MOBILE');
				}else if(this.tablet()) {
					d.push('tablet',sO.Platform);//console.log('//TABLET');
				}else{
					if($.isFunction(this[sO.Device])){
						d.push(sO.Deploy,sO.Platform);//console.log('//DEPLOY');
					}else{
						d.push(sO.Device,sO.Platform);//console.log('//DESKTOP');
					}
				}
				if(this.ios()) {
					d.push('ios');
			...
			if(config.footer[lai.store.query.page]){
				if(lai.todo.footerChanged != lai.store.query.page){
					container.replaceWith(this.fn({footer:config.footer[lai.store.query.page]}).html(null,null,true)).promise().done(function(){
						...
						lai.todo.Orientation=true;
						console.log('footer 1',config.css.currentPage);
					});
				}
				...
				container.replaceWith(this.fn({footer:config.body.footer}).html(null,null,true)).promise().done(function(){
					...
					$(config.css.currentPage).addClass(config.css.active);
					lai.todo.Orientation=true;
					console.log('footer 2',config.css.currentPage);
				});
			}else{
				container.find(this.is(config.css.active).class).removeClass(config.css.active).promise().done($(config.css.currentPage).addClass(config.css.active));
			}
	...
	*/
	...
	fA:{
		...
		chapter:{
			...
			html:function(container,bID){
				var ol=$(h.ol,{class:'list-chapter'}).appendTo(container);
			...
			list:function(e){
				...
				if(container.is(':hidden')){
					...
					this.html(container.fadeIn(200).find(z.is(z.get(x).class()[1]).id).empty(),lai.store.query.book).promise().done(function(){
			...
			book:function(bID){
				//TODO
				var self=this;
				return $(h.span).html(bible.info[bID].c).on(sO.Click,function(){
					//console.log('click');
