(function($,uA){
var z='laisiangtho', version='1.9.74.2015.7.18';
...
return z={
		...
		var l7=[], l8={}, obj={
			go:function(){
				...
				if(lai.store.lang[bID].id){
					console.log(lai.store.lang[bID]);
					console.log('this.next');
					//lai[bID].lang=lai.store.lang[bID]; this.next();
				}else{
					//this.json(bID,this.next);
					console.log('this.json',lai.store.query.bible,sO.Ready);
				}
				*/
				if(lai.store.lang[bID].id){
					lai[bID].lang=lai.store.lang[bID];
					sO.SplashLang.html(lai[bID].lang.info.name).promise().done(function(){
	...
	fA:{
		...
		chapter:{
			...
			list:function(e){
				var y=$(e.target), con=y.parent().children().eq(1);
				if(con.is(':hidden')){
					y.addClass(config.css.active);
					this.html(con.fadeIn(200).find(z.is(z.get(y).class()[1]).id)).promise().done(function(){
						$(document).on(sO.Click,function(x){
							if(con.is(':visible') && !$(x.target).closest(con).length && !$(x.target).closest(y).length)con.fadeOut(200).promise().done(y.removeClass(config.css.active));
						});
					});
				}else{
					con.fadeOut(200).promise().done(y.removeClass(config.css.active));
				}
			},
			next:function(){
		...
		lookup:{
			...
			setting:function(e){					
				var y=$(e.target), con=y.parents('div').children().eq(1);
				if(con.is(':hidden')){
					y.addClass(config.css.active);
					this.html(con.fadeIn(200).find(z.is(z.get(y).class()[1]).id)).promise().done(function(){
						$(document).on(sO.Click,function(x){
							if(con.is(':visible') && !$(x.target).closest(con).length && !$(x.target).closest(y).length)con.fadeOut(100).promise().done(y.removeClass(config.css.active));
						});
					});
				}else{
					con.fadeOut(200).promise().done(y.removeClass(config.css.active));
