...
return z={
	...
	fA:{
		...
		container:{
			msg:function(e){
				console.log(e);
				MsgInfo=e;
				//MsgInfo=e; e.bind(sO.Orientation.change, function(evt){evt.stopPropagation(); z.utility.Orientation();})
			}
		}
	},
	regex:function(q){
	...
	bible:function(){
				...
				$(h.p).append(
					$(h.span,{class:isAvailable}).on(sO.Click,function(){
						...
						if(lai.todo.bibleOption==bID){
							loaded(); delete lai.todo.bibleOption;
		...
		this.containerMain.append($(h.div,{class:'delete'}).css({height:500}).html('$(this).html(this is testthis is testthis is testthis is testthis is testthis); '));
		var loading=function(q){
			if(MsgInfo.is(":visible")){
				MsgInfo.html(q.msg);
			}else{
				$('body').addClass($.merge(['working'],($.isArray(q.class)?q.class:[])).join(' ')).promise().done(function(){
					MsgInfo.html(q.msg).slideDown(300);
				});
			}
		};
		var loaded=function(){
			MsgInfo.slideUp(200).empty().promise().done($('body').removeAttr('class'));
		};
	},
	book:function(){
