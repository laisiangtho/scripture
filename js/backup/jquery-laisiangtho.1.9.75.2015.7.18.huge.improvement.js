(function($,uA){
var z='laisiangtho', version='1.9.75.2015.7.18';
...
return z={
		...
		var device={
			...
			Orientation:{
				handle:function(){
					...
					if(sO.Ready)z.utility.Orientation();
				},
				init:function(){
	...
	meta:function(data){
		return {
			data:[{type:'script',name:'localforage.min'},{type:'script',name:'Sortable.min'},{type:'script',name:'data.bible'},{type:'script',name:'data.config'},{type:'link',name:'laisiangtho',dir:'fonts/fontello/'}],
	...
	ready:function(){
		for(var q in sO.Q){
			var x=($.type(sO.Q[q]) == 'object')?Object.keys(sO.Q[q])[0]:sO.Q[q];
			this[0](x.split(' '),sO.Q[q][x]);
		}
	},
	0:function(x,q){
	...
	watch:function(x){
		return {
			id:function(){z[0](z[1]($(this)).id);},
			class:function(){z[0](z[1]($(this)).class,$(this));},
			go:function(q){z[0](x,q);}
		};
	},
	action:function(){
		...
		return {
			...
			run:function(){
				...
				if(x[i[0]]){
					...
					if(i[1] && $.isFunction(x[i[0]][i[1]]))y.off().on(sO.Click,function(e){x[i[0]][i[1]](e);});//e.stopImmediatePropagation();
				}
			},
	...
	database:{
		...
		add:function(q){
			//console.log('database add',q.table);
			return localforage.setItem(q.table, q.data);
		},
		get:function(q){
			//console.log('database get',q.table);
			return localforage.getItem(q.table);
		},
		put:function(q){
			//console.log('database put',q.table);
			return localforage.setItem(q.table, q.data);
		},
		read:function(q){
			localforage.keys(q);
		...
		var l7=[], l8={}, obj={
			go:function(){
				...
				if(lai.store.lang[bID].id){
					lai[bID].lang=lai.store.lang[bID];
					$('p').html(lai[bID].lang.info.name).delay(100).promise().done(function(){
						z.content({bible:bID,reading:reading()}).xml(function(response){
							lai.store.lang[bID].offline=response.status; obj.next();
						}).read();
					});
				}else{
			...
			next:function(){
				if(l7.length){
					...
					delete sO.SplashLang;
					z.loading=function(){
						this.fn(config.loader).html($('body')).on(sO.Click,function(){
							MsgInfo.text(lai[lai.store.query.bible].lang.l.PleaseWait).trigger(sO.Orientation.change);
						}).promise().done(function(){
							sO.working=this;
						});
					};
					z.loaded=function(){
						if(sO.working)sO.working.remove().promise().done(function(){
							delete sO.working; MsgInfo.empty().trigger(sO.Orientation.change);
						});
					};
			...
			json:function(bID,callback,x){
				...
				request.done(function(j){
					var lID=j.info.lang || config.language.id;
					j.info.lang=lID;
					sO.SplashLang.html(j.info.name).attr({title:lID});
					var prepare=function(lC,lN){
						...
						return {
							...
							next:function(){
								lai[bID].lang=$.extend(lai.store.lang[bID],this.merge());
								z.content({bible:bID,reading:true}).xml(function(response){
									lai.store.lang[bID].offline=response.status;
									callback();
								}).read();
							}
						};
			...
			available:function(j){
				if(j){lai.store.lang=z.array(config.bible.available,Object.keys(j)).merge().unique().reduce(function(o,v,i){o[v]= {index:(j[v].index||j[v].index==0)||i};return o;}, {});}
					else{lai.store.lang=config.bible.available.reduce(function(o,v,i){o[v]={index:i};return o;}, {});}
			}
		};
		sO.SplashLang.html('getting Database ready').attr({class:'icon-database'});
		this.database.init(function(sql){
			sO.SplashLang.html('getting Configuration ready').attr({class:'icon-config'});
			sql.get({table:config.store.info}).then(function(storeInfo){
				sO.SplashLang.html('getting Language ready').attr({class:'icon-language'});
				sql.get({table:config.store.lang}).then(function(storeLang){
					sO.SplashLang.attr({class:'icon-flag'});
					if(storeLang){
						if(storeInfo && storeInfo.build == config.build){
							sO.Ready=3; lai.store.lang=storeLang; process_query();
						}else{
							sO.Ready=2; obj.available(storeLang); process_query();
						}
					}else{
						sO.Ready=1; obj.available(); process_query();
					}
				});
	...
	init:function(){
		...
		/*
		MsgInfo.bind("DOMSubtreeModified",function(){
			//console.log('DOMSubtreeModified,DOMNodeInserted DOMNodeRemoved');
			//z.utility.Orientation();
		});
		*/
	},
	utility:{
		...
		Orientation:function(){
			$(config.css.content).css({'top':$(config.css.header).outerHeight(),'bottom':$(config.css.footer).outerHeight()}); console.log('Orientation');
		},
		Analytics:function(){
			if(z.analytics)z.analytics.sendPage(lai.store.query.page);
		},
		Closest:function(container,x,y){
			if(container.is(':visible') && !$(x.target).closest('#working, .win').length && !$(x.target).closest(container).length && !$(x.target).closest(y).length) return true;
		}
	},
	fA:{
		about:{
			version:function(){
				$(h.div,{id:'working',class:'version'}).append(
					...
					if(!$(evt.target).closest('#popup').length)$('#clickme').fadeOut(50).fadeIn(50);
				});
			}
		},
		chapter:{
			...
			nameCurrent:function(e){
				e.html(z.utility.Num(lai.store.query.chapter)); if(lai.todo.ActiveChapter)lai.todo.ActiveChapter.addClass(config.css.active);
			},
			nameNext:function(e){
			...
			html:function(container){
				var ol=$(h.ol,{class:'list-chapter'}).appendTo(container.empty()); delete lai.todo.ActiveChapter;
				$.each(bible.info[lai.store.query.book].v,function(chapter,verse){
					chapter++;
					$(h.li,{id:chapter,class:(lai.store.query.chapter==chapter?config.css.active:'')}).on(sO.Click,function(){
			...
			list:function(e){
				var y=$(e.target), container=y.parent().children().eq(1);
				if(container.is(':hidden')){
					y.addClass(config.css.active);
					this.html(container.fadeIn(200).find(z.is(z.get(y).class()[1]).id)).promise().done(function(){
						$(document).on(sO.Click,function(x){
							if(z.utility.Closest(container,x,y))container.fadeOut(200).promise().done(y.removeClass(config.css.active));
						});
					});
				}else{
					container.fadeOut(200).promise().done(y.removeClass(config.css.active));
				}
			},
			next:function(){
		...
		lookup:{
			...
			setting:function(e){					
				var y=$(e.target), container=y.parents('div').children().eq(1);
				if(container.is(':hidden')){
					y.addClass(config.css.active);
					this.html(container.fadeIn(200).find(z.is(z.get(y).class()[1]).id)).promise().done(function(){
						$(document).on(sO.Click,function(x){
			...
			msg:function(e){
				MsgLookup=e;
				if(lai.store.query.result > 0){e.text(z.utility.Num(lai.store.query.result)).attr('title',lai.store.query.q);}else{e.empty();}
			}
		},
		container:{
			msg:function(e){
				MsgInfo=e;
				e.bind(sO.Orientation.change, function(evt){evt.stopPropagation(); z.utility.Orientation();})
			}
		}
	},
	regex:function(q){
				...
				var xml={
					read:function(){
						if($.isEmptyObject(lai[q.bible].bible)){
							if(sO.isCordova){
								window.resolveLocalFileSystemURL(o.local, xml.local.content, function(){
									xml.done({msg:'to Local',status:false});
								});
							}else{
								db.get({table:q.bible}).then(function(storeBible){
									if(storeBible){
										if($.isEmptyObject(storeBible)){
											xml.done({msg:'to Store',status:false});
										}else{
											if(q.reading==q.bible){
												lai[q.bible].bible=storeBible; xml.done({msg:'from Store',status:true});
											}else{
												xml.done({msg:'to Store',status:true});
											}
										}
									}else{
										xml.done({msg:'to Store',status:false});
							...
							xml.done({msg:'from Object',status:true});
						}
					},
					remove:function(){
						if(sO.isCordova){
							window.resolveLocalFileSystemURL(o.local, xml.local.remove, function(){
					...
					get:function(){
						if($.isEmptyObject(lai[q.bible].bible)){
							...
							this.done({msg:'from Object',status:true});
						}
					},
					load:function(x){
						$.ajax({
							...
							beforeSend:function(xhr){
								z.loading();
								MsgInfo.text(lD.l.Downloading).trigger(sO.Orientation.change);
							...
							xhr:function(){
								...
								xhr.addEventListener("progress", function(evt){
									if(evt.lengthComputable) {
										...
										MsgInfo.text(lD.l.PercentLoaded.replace("{Percent}", z.utility.Num(Percentage)));
									}
								}, false);
								/*
								//Upload progress
								XMLHttpRequest.upload.addEventListener("progress", function(evt){
									if(evt.lengthComputable) {
										...
										MsgInfo.text(lD.l.PercentLoaded.replace("{Percent}", Percent));
									}
								}, false); 
								*/
								return xhr;
					...
					local:{
						...
						content:function(fileEntry){
							...
							if(q.reading){
								if(q.reading==q.bible){ reading();}else{ xml.done({msg:'to Store',status:true});}
							}else{
								reading();
							}
						},
					...
					job:{
						...
						if(q.reading){
							...
							if(lai.store.lang[q.bible].offline===false){
								z.loaded();
								lai.store.lang[q.bible].offline=true; db.UpdateLang();
							}
							if(m.status===false){z.utility.remove.lai(q.bible);}
							callback(m);
						}
						/*
						if(lai.store.lang[q.bible].offline===true){
							callback(m);
						}else{
							if(lai.store.lang[q.bible].offline==false){
								z.loaded();
						...
						*/
						...
						if(lD.offline===false){
							z.loaded();
							lai.store.lang[q.bible].offline=true;
							lai.store.lang[q.bible].offline=true; db.UpdateLang();
						}
						if(m.status===false){z.utility.remove.lai(q.bible);}
						*/
					...
					bible:function(dQ){
						...
						$.each(dQ,function(bID,data){
							setTimeout(function(){
								...
								if(book){
									if(lai.todo.pause)return MsgInfo.text(lai[q.bible].lang.l.Paused);
	...
	bible:function(){
		...
		config.bible.available.forEach(function(bID){
			var lang=lai[bID].lang.info, isOffline=lai[bID].lang.offline, availableClass=(isOffline)?config.css.available:config.css.notAvailable,
			offlineClass='icon-logout offline',onlineClass='icon-cloud offline', isAvailable=(isOffline)?offlineClass:onlineClass;
			$(h.li,{id:bID,class:(lai.store.query.bible==bID)?config.css.active+' '+availableClass:availableClass}).html(
				$(h.p).append(
					$(h.span,{class:isAvailable}).on(sO.Click,function(){
						var x=$(this), li=x.parents('li');
						//console.log(bID);
						/*
						$('.dynamic').fadeToggle( "fast", function() {
							$(this).append('this is test');
						});
						*/
						$('.dynamic').slideToggle(300, function() {
							$(this).html('this is testthis is testthis is testthis is testthis is testthis');
						});
						/*
						if(li.hasClass(config.css.notAvailable)){
							//var id=z.get(li).id()[0];//$.extend({},lai.store.query,{bible:id})
							...
							MsgInfo.empty().append(
								$(h.span).text(lai[bID].lang.l.WouldYouLikeToRemove.replace('{is}',x.parent().children('a').text())),
								$(h.span,{class:'icon-thumbs-up-alt'}).on(sO.Click,function(evt){
									...
									z.content({bible:bID}).xml(function(response){
										if(response.status){
											lai[bID].lang.offline=false;
											delete lai[bID].bible;
										}
										MsgInfo.empty().trigger(sO.Orientation.change);
									}).remove();
									//MsgInfo.empty().trigger(sO.Orientation.change);
									//delete lai[bID].bible;
									//lai[bID].lang.offline=false;
								...
								$(h.span,{class:'icon-thumbs-down-alt'}).on(sO.Click,function(evt){
									evt.preventDefault(); evt.stopImmediatePropagation();
									MsgInfo.empty().trigger(sO.Orientation.change);
								})
							).trigger(sO.Orientation.change);
						}
						*/
					}),
			...
			Sortable.create(document.getElementById(this.get(0).id), {
				...
				onUpdate:function(evt){
					ol.children().each(function(i,e){
						var id=$(e).get(0).id, index=$(e).index();
						$.extend(lai.store.lang[id],{index:index});
					}).promise().done(function(){
	...
	book:function(){
		...
		/*
		div.append(
			$(h.h6,{text:'ClickME'}).on(sO.Click,function(){
				$('.dynamic').slideToggle( "slow", function() {
					$(this).append('this is test');
				});
			})
		);
