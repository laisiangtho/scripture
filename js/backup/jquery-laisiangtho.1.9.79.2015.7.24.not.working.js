(function($,uA){
var z='laisiangtho', version='1.9.79.2015.7.23';
...
return z={
		...
		return {
			...
			run:function(){
				...
				if(x[i[0]]){
					...
					if(i[1] && $.isFunction(x[i[0]][i[1]]))y.on(sO.Click,function(e){x[i[0]][i[1]](e);});
				}
			},
	...
	load:function(){
		...
		$('p').addClass(config.css.active).html(config.version).promise().done(function(){
			//$(document).on(sO.Click,function(evt){ console.log('123');evt.stopPropagation();});
		});
		...
		var l7=[], l8={}, obj={
			...
			go:function(){
				...
				if(lai.store.lang[bID].id){
					$("p").html(lai.store.lang[bID].info.name).delay(200).promise().done(function(){
			...
			json:function(bID,callback,x){
						...
						return {
							...
							next:function(){
								...
								z.content({bible:bID,reading:obj.reading(bID)}).xml(function(response){
									...
									$("p").html(lID).attr({class:'icon-database'}).delay(200).promise().done(callback());
								}).read();
								/*
								$("p").html(lai.store.lang[bID].info.name).delay(200).promise().done(function(){
									z.content({bible:bID,reading:obj.reading(bID)}).xml(function(response){
										lai.store.lang[bID].offline=response.status; callback();
									}).read();
								});
								*/
							}
				...
				request.fail(function(jqXHR, textStatus){
					if(api){
						if(x){
							z.utility.remove.bible(bID,function(){
	...
	init:function(){
		...
		z.utility.Body().keydown(function(e){
	...
	utility:{
		...
		Working:function(q){
			if(MsgInfo.is(":hidden")){
				$('body').addClass(config.css.working+(q.wait?' '+config.css.wait:'')).promise().done(MsgInfo.slideDown(200));
			}else if(q.wait){
				$('body').addClass(q.wait);
			}
			return MsgInfo.html(q.msg);
		},
		...
		Body:function(){
			return $('body').attr({id:lai.store.query.page}).removeClass();
		},
	...
	rootFileSystem:function(){
		if(sO.isCordova && window.LocalFileSystem){
			this.requestFileSystem(
				function(dir){
					sO.FileDirectory=dir.root;//fileSystem
				},
				function(error){
	...
	fA:{
		...
		chapter:{
			...
			text:function(x){
				var q=this.get[x]();
				return lai.store.lang[lai.store.query.bible].b[q.book]+' '+z.utility.Num(q.chapter);
			},
			nameCurrent:function(e){
				e.html(z.utility.Num(lai.store.query.chapter)); 
				if(lai.todo.ActiveChapter){
					//TODO if(sO.isCordova)-> REMOVE
					lai.todo.ActiveChapter.addClass(config.css.active);
		...
		fn={
			xml:function(callback){
				...
				var xml={
					...
					rootFileSystem:function(callback){
						this.requestFileSystem(
							function(fileSystem){
								sO.FileDirectory=fileSystem.root;
								callback(sO.FileDirectory);
							},
							function(error){
								delete sO.FileDirectory;
								callback(null,error);
					...
					*/
					read:function(){
						if($.isEmptyObject(lai[q.bible].bible)){
							// TODO
							if(sO.isCordova){
								z.resolveFileSystem(o.local,xml.local.content,function(){
										xml.done({msg:'to Local',status:false});
									}
								);
								/*
								if(sO.FileDirectory){
									sO.FileDirectory.getFile(o.filename, {create:false}, xml.local.content, function(error){
										xml.done({msg:'to Local',status:false});
									});
								}else{
									//TODO -> BACKUP
									z.requestFileSystem(function(fileSystem){
										sO.FileDirectory=fileSystem.root;
										sO.FileDirectory.getFile(o.filename, {create:false}, xml.local.content, function(){
											xml.done({msg:'to Local',status:false});
										});
									}, function(error){
										xml.done({msg:'to Local',status:false});
					...
					remove:function(){
						...
						if(sO.isCordova){
							...
							z.requestFileSystem(function(dir){
								dir.root.getFile(o.filename, {create:false}, xml.local.remove, function(error){
							...
							/*
							if(sO.FileDirectory){
								sO.FileDirectory.getFile(o.filename, {create:false}, xml.local.remove, function(){
									callback({status:true});
								});
							}else{
								//TODO -> BACKUP
								z.requestFileSystem(function(fileSystem){
									sO.FileDirectory=fileSystem.root;
									sO.FileDirectory.getFile(o.filename, {create:false}, xml.local.remove, function(error){
										callback({status:true});
									});
								}, function(error){
									callback({status:true});
					...
					get:function(){
						if($.isEmptyObject(lai[q.bible].bible)){
							if(sO.isCordova){
								//TODO
								z.utility.Working({msg:lD.l.PleaseWait,wait:true});
								z.resolveFileSystem(o.local, xml.local.content, xml.local.download);
							}else{
								db.get({table:q.bible}).then(function(storeBible){
									if(storeBible){
					...
					job:{
																	...
																	if(d2.length == c+1){
																		if(d3.length == v+1){
																			if(sO.isCordova){
																				...
																				db.put({table:q.bible,data:lai[q.bible].bible}).then(xml.done({msg:'Stored',status:true}));
																			}
																		}
																	}
																}
						...
						if(q.reading){
							...
							if(m.status===false){z.utility.remove.bible(q.bible);}
							callback(m);
						}
					}
				}; return xml;
	...
	bible:function(){
		...
		config.bible.available.forEach(function(bID){
			var lang=lai.store.lang[bID].info, isOffline=lai.store.lang[bID].offline, availableClass=(isOffline)?config.css.available:config.css.notAvailable,
				...
				$(h.p).append(
					$(h.span,{class:isAvailable}).on(sO.Click,function(evt){
						...
						if(lai.todo.bibleOption==bID){
							...
							z.utility.Working({
								msg:$(h.div).append(
									$(h.span).text(lai.store.lang[bID].l.WouldYouLikeToRemove.replace('{is}',x.parent().children('a').text())),
									$(h.span,{class:'icon-thumbs-up-alt'}).on(sO.Click,function(evt){
										...
										z.content({bible:bID}).xml(function(response){
											z.utility.Done(function(){
												delete lai.todo.bibleOption;
												if(response.status){
													lai.store.lang[bID].offline=false; delete lai[bID].bible;
													db.UpdateLang();
												}
											});
										}).remove();
