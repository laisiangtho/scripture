...
return z={
		...
		var l7=[], l8={}, obj={
			...
			json:function(bID,callback,x){
				...
				request.done(function(j){
					...
					j.info.lang=lID;
					MsgInfo.html(j.info.name).attr({title:lID});
	...
	utility:{
		...
		Working:function(q){
			MsgInfo.html(q.msg);
			if(MsgInfo.is(":hidden")){$('body').addClass(config.css.working+(q.wait?' '+config.css.wait:'')).promise().done(MsgInfo.slideDown(300));}
			else if(q.wait){$('body').addClass(q.wait);}
			if(q.callback)q.callback(MsgInfo);
		},
		Done:function(){
				...
				$(h.p).append(
					$(h.span,{class:isAvailable}).on(sO.Click,function(){
						...
						if(lai.todo.bibleOption==bID){
							z.utility.Done(); delete lai.todo.bibleOption;
