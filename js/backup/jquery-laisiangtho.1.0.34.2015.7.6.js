(function($,win,doc,uA){
var z='laisiangtho', version='1.0.34.2015.7.6';
...
return z={
		...
		var device={
			...
			*/
			orientation:{
				...
				handle:function(){
					if(device.orientation.landscape()){$(docElement).attr({class:sO.Orientation.landscape});} else {$(docElement).attr({class:sO.Orientation.portrait});}
				}
			},
			get:function(){
				var d=[sO.Layout];
	...
	ld:function(data){
		this.ld={
			...
			start:function(){
				$.merge(z.ld.data, data||{});
				z.ld.src(z.ld.data);
			},
			src:function(meta){
				var x=meta.shift(), y=x.type, url=(x.dir||this.type[y].dir)+x.name+this.type[y].extension, req=doc.createElement(y);
				for(var i in this.type[y].attr){req[i]=this.type[y].attr[i]||url;}
				req.onload=function(){
					$('li').html(x.name);
					if(meta.length){z.ld.src(meta);}else{z.extend();}
				};
	...
	load:function(){
		var l7=[];
		h=z.utility.Html();
		lai={store:{note:false,lang:{},query:{},lookup:{setting:{},book:{}}},previous:{},todo:{}};
		$('p').prepend(config.version+' : ');
		...
		var load={
			...
			get:function(){
				this.json(l7.shift(),this.next);			
			},
			go:function(){
				var x=l7.shift(); load.id(x);
				if(lai.store.lang[x].id){
					lai[x].lang=lai.store.lang[x]; load.next(this.go);
				}else{
					load.json(x,load.next,true);
				}
			},
			next:function(callback){
				if(l7.length){
					if(callback){callback();}else{load.go();}
				}else{
					z.load={
						loading:function(){
						...
						loaded:function(){$('#working').remove();},
			...
			json:function(x,callback,y,aP){
				var action=(y?'put':'add'), o=z.utility.Url(config.id,x,config.file.lang);
				var request=$.ajax({url:(aP)?aP+o.url:o.url,dataType:o.data,contentType:o.content,cache:false});
				var MsgLang=$('li');
				request.done(function(j){
					MsgLang.html(j.info.name);
					var langID=j.info.lang || config.language.id;
					function prepare(lC,lN){
						var lang={};
						return {
							is:{
								id:function(e){
									lai.store.lang[x][e]=x;
								},
								name:function(e){
									lai.store.lang[x][e]={};
									for(var i in lC[e]){
										...
										$.merge(jB,jN);
										lai.store.lang[x][e][i]=$.unique(z.array(lC[e][i]).merge(jB).data);
									}
								}
							},
							obj:function(){
								for(var f in lC){
									if(this.is[f]){
										...
										lai.store.lang[x][f]=(lN[f])?$.extend({},lC[f],lN[f]):lC[f];
									}
								}
								return lai.store.lang[x];
							},
							next:function(){
								lai[x].lang=this.obj();
								callback();
					...
					if(load.lang[langID]){
						prepare(load.lang[langID],j).next();
					}else{
						var o=z.utility.Url('lang',langID,config.file.lang);
						var request_lang=$.ajax({url:o.url,dataType:o.data,contentType:o.content,cache:false});
						request_lang.done(function(langauge){
							load.lang[langID]=prepare(config.language,langauge).obj();
							prepare(load.lang[langID],j).next();
						});
						request_lang.fail(function(jqXHR, textStatus){
				...
				request.fail(function(jqXHR, textStatus){
					if(api){
						if(aP){
							load.remove(x,callback);
						}else{
							load.json(x,callback,y,api);
						}
					}else{
						load.remove(x,callback);
					}
			...
			remove:function(x,callback){
				z.utility.remove.lai(x,function(){
					l7.splice(l7.indexOf(x), 1);
					callback();
				});
			},
			lang:{},
			available:function(x){
				if(x){
					lai.store.lang=z.array(config.bible.available,Object.keys(x)).merge().unique().reduce(function(o,v,i){o[v]= {index:x.index||i};return o;}, {});
				}else{
					lai.store.lang=config.bible.available.reduce(function(o,v,i){o[v]={index:i};return o;}, {});
				}
				return lai.store.lang;
			}
		};
		this.database.init(function(sql){
			sql.get({table:config.store.info}).then(function(storeInfo){
				sql.get({table:config.store.lang}).then(function(storeLang){
					if(storeLang){
						if(storeInfo && storeInfo.build == config.build){
							...
							load.available(storeLang);process_query();//console.log('ONUPDATE');
						}
					}else{
						load.available();process_query();//console.log('ONINSTALL');
					}
				});
				...
				function process_trigger(){
					...
					if(storeInfo && storeInfo.build == config.build){load.go();}
					else{sql.add({table:config.store.info,data:{build:config.build,version:config.version}}).then(load.go());}
				};
			});
		});
	},
	...
	utility:{
		...
		Url:function(l,x,y){
			//fI
			return {url:z.fn([l,47,x,46,y]).url(), data:y, content:z.fn(['application',47,y,'; charset=utf-8']).string()};
		},
		Num:function(n,l){
			//fD
	...
	fA:{
		...
		lookup:{
			...
			msg:function(e){
				...
				z.MsgLookup=e;
				if(lai.store.query.result > 0){e.html(lai.store.query.result).attr('title',lai.store.query.q);}else{e.empty();}
			}
		},
		container:{
			msg:function(e){
	...
	bible:function(){
		...
		config.bible.available.forEach(function(id){
			var lang=lai[id].lang.info;
			$(h.li,{class:(lai.store.query.bible==id)?config.css.active:id}).html(
				$(h.a,{href:p+$.param({bible:id})}).html(lang.name)
			).append(
				$(h.p).html(lang.des)
			).appendTo(ol);
		});
		/*
		this.db.bible.get('judson').onsuccess=function(e){
			if(e.target.result){
				//localStorage.setItem('book_66',JSON.stringify(e.target.result.book[66]));
				//$('ol').prepend($(h.li).html(JSON.stringify(e.target.result.book[66].chapter[22].verse.v20)))
				$('ol').prepend($(h.li).html(JSON.stringify(e.target.result.book[66].chapter[22].verse.v20)));
				//localStorage.setItem('19',JSON.stringify(e.target.result));
				console.log('localstorge',localStorage.length);
				
			}else{
				console.log('no e.target.result');
			}
		};
		*/
		//z.db.bible.add(lai[lai.store.query.bible].bible);
		//this.db.deleteDatabase();
/*
			if (d) {
				localStorage.setItem('config',JSON.stringify(d));
			} else {
				return $.parseJSON(localStorage.getItem('config'));
			}
*/
	},
	book:function(){
