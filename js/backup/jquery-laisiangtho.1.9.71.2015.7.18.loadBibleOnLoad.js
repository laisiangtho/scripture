(function($,win,doc,uA){
var z='laisiangtho', version='1.9.71.2015.7.17';
...
return z={
	agent:function(){
		var device={
			...
			Orientation:{
				handle:function(){
					$(win.document.documentElement).attr({class:((win.innerHeight / win.innerWidth) < 1)?sO.Orientation.landscape:sO.Orientation.portrait});
	...
	load:function(){
		...
		$('p').addClass(config.css.active).promise().done(function(){
		...
		var l7=[], l8={}, obj={
			go:function(){
				...
				if(lai.store.lang[bID].id){
					...
					MsgLang.html(lai[bID].lang.info.name).promise().done(function(){
