(function($,uA){
var z='laisiangtho', version='1.9.72.2015.7.18';
...
return z={
	...
	load:function(){
		...
		$('p').addClass(config.css.active);
		$('h1').attr({title:config.version});
		var l7=[], l8={}, obj={
			go:function(){
			...
			json:function(bID,callback,x){
				...
				request.done(function(j){
					sO.SplashLang.html(j.info.name);
					var langID=j.info.lang || config.language.id;
					sO.SplashLang.attr({title:langID});
				...
				sql.get({table:config.store.lang}).then(function(storeLang){
					if(storeLang){
						if(storeInfo && storeInfo.build == config.build){
							...
							lai.store.lang=storeLang; process_query();
						}else{
							//console.log('ONUPDATE');
							obj.available(storeLang); process_query();
						}
					}else{
						//console.log('ONINSTALL');
						obj.available(); process_query();
					}
				});
