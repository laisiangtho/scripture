(function($,uA){
var z='laisiangtho', version='1.9.80.2015.7.25';
	...
	*/
				...
				var xml={
						...
						if($.isEmptyObject(lai[q.bible].bible)){
							if(sO.isCordova){
								z.requestFileSystem(function(fileSystem){
									fileSystem.root.getFile(o.filename, {create:false}, xml.local.reading, function(){
					...
					get:function(){
						...
						if($.isEmptyObject(lai[q.bible].bible)){
							if(sO.isCordova){
								//TODO
								z.resolveFileSystem(o.local, xml.local.getting, xml.local.download);
							}else{
								db.get({table:q.bible}).then(function(storeBible){
									if(storeBible){
							...
							//this.done.getting({msg:'from Object',status:true});
	...
	bible:function(){
		...
		config.bible.available.forEach(function(bID){
			...
			offlineClass='icon-logout offline',onlineClass='icon-cloud offline', isAvailable=(isLocal)?offlineClass:onlineClass;
				...
				$(h.p).append(
					$(h.span,{class:isAvailable}).on(sO.Click,function(evt){
						...
						if(lai.todo.bibleOption==bID){
							...
							z.utility.Working({
								/*
								msg:$(h.div,{class:'data-dialog'}).append(
								...
								*/
								msg:$(h.ul,{class:'data-dialog'}).append(
										$(h.li).append(
											$(h.p).text(lai.store.lang[bID].l.WouldYouLikeToRemove.replace('{is}',x.parent().children('a').text()))
										),
										$(h.li).append(
