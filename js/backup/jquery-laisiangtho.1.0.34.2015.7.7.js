...
return z={
	...
	utility:{
		...
		Url:function(l,x,y){
			//fI
			return {url:z.fn([l,47,x.join('/'),46,y]).url(), data:y, content:z.fn(['application',47,y,'; charset=utf-8']).string()};
		},
		Num:function(n,l){
			//fD
																	...
																	if(d2.length == c+1){
																		if(d3.length == v+1){
																			if(jobList.length){
																				...
																				/*
																				localforage(q.bible+'.'+i2,lai[q.bible].bible[t2][i2],function(err,val){
																					if(err)xml.msg(err);
																					xml.next();
																				});
																				*/
