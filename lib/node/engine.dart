...
mixin Engine on Configuration {
  ...
  Future initCollection() async {
    if (this.collection == null) {
      ...
      await docsExists(fileName).then((bool yes) async{
        if (yes){
          await docsRead(fileName).then((response) => this.parseCollection(response));
        } else {
          await loadBundleAsString(assetsBookJSON).then((response) => this.parseCollection(response));
          await writeCollection();
        }
      });
    }
    if (this.identify.isEmpty){
      this.identify = this.collection.book.firstWhere((i) => i.available > 0,orElse: () => collection.book.first).identify;
    }
    this.isReady = this.collection != null;
  }
  ...
  Future<Collection> updateCollection() async {
    return await requestHTTP('nosj.koob/retsam/elbib/ohtgnaisial/moc.tnetnocresubuhtig.war//:sptth'.split('').reversed.join()).then((response) async{
      ...
      await this.parseCollection(parsed);
      await writeCollection();
      return collection;
    });
  }
